...
const NuevaMaquina = ({ ...props }) => {
    ...
    const [caracteristicas, setCaracteristicas] = useState([{ "clave": "Modelo", "valor": "" }])
    const [partes, setPartes] = useState([])
    const [categoria, setCategoria] = useState('')
    const [ubicacion, setUbicacion] = useState('')
    const [vida_util, setVida] = useState(0)
    const [fecha_adquirido, setFecha] = useState('')
        ...
        if(!band){
            if(fecha_adquirido <= fecha){
                if(vida_util > 0){
                    ...
                    const { estado, message } = data.insertarMaquina;
                    if (estado) {
    ...
    const FichaParte = ({ item }) => {
        console.log(item)
        return (
            <div className='rounded bg-white shadow px-1' style={{ width: 250, maxWidth: 250 }}>
...
export default withRouter(NuevaMaquina)