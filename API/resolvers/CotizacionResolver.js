...
export default{
    Query:{
        obtenerCotizaciones: async (_, {}) => {
            try{
                const result = await Cotizacion.find({status: 'ACTIVO'}).populate('formula').populate('presentacion').populate('cliente').populate('elementos').populate('capsula');
                return result;
            }catch(error){
                return error
            }
        ...
        obtenerCotizacion: async (_, {id}) => {
            try{
                const result = await Cotizacion.findById(id).populate('formula').populate('presentacion').populate('cliente').populate('elementos').populate('capsula');
                return result;
            }catch(error){
                return error
            }
